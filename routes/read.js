    ...
    .post('/user/query', async (ctx) => {
        try {
            const rows = await new Promise((resolve, reject) => {
              connection.query('SELECT * FROM user.personal', function (err, rows) {
                if (err) {
                  reject(err);
                } else {
                  resolve(rows); 
                  console.log(JSON.parse(JSON.stringify(rows))); //用 JSON.stringify() 將物件或陣列轉換成字串，就可以正確顯示所有資料。而使用 JSON.parse() 將字串轉換回物件或陣列，就可以在程式碼中繼續操作該資料。
                }
              });
            });
            ctx.body = rows;
  ...
  .post('/user/query/:id', async (ctx) => {
    try {
      ...
      if(!member){
        ...
        member = JSON.stringify(rows[0]); // 取出第一筆資料並轉成 JSON 格式
